...
import com.mrpan.wechat.user.utis.UserConvertUtils;
...
public class UserConn {
    ...
    //批量获取用户信息
    private static String UNIOID_BATCHGET_PATH="https://api.weixin.qq.com/cgi-bin/user/info/batchget";
    //获取用户列表
     ...
     * @param request		批量请求参数集合
     * @return
     */
    public static String batchgetUserList(String access_token,BatchGetRequest request){
        Connection conn = new Connection();
        TreeMap<String,String> params = new TreeMap<String,String>();
        params.put("access_token",access_token);
        String postData = ConvertJsonUtils.toJsonString(request);
        String result = conn.HttpsDefaultExecute("POST",UNIOID_BATCHGET_PATH, params, postData);
        return result;
    }

    /**
     * 根据授权token获取该公众号下的所有用户信息,该方法有严重的性能问题，请在后面重构
     * 经测试,1490个用户,拉取时间为26秒左右,
     * @param accessToken	授权token
     * @return		List<WeixinUser> 微信用户详细信息
     */
    public static List<WeixinUser> batchgetUserList(String accessToken, String lang){
        String result =getUserOpenList(accessToken, "");
        ...
        if(result1.isSuccess()){
            ...
            for(int i =0;i<size;i++){
                ...
                List<String> newList = info.getOpenid().subList(fromIndex,toIndex);		//每次截取两个范围之间的数据获取详细数据
                WechatResult result2 = UserConvertUtils.convertRequestParams(newList,lang);
                if(result2.isSuccess()){
                    BatchGetRequest request2 = (BatchGetRequest) result2.getObj();
                    String data = UserConn.batchgetUserList(accessToken, request2);
     ...
     * @return
     */
    public static String getUserOpenList(String accessToken,String next_openid){
        Connection conn = new Connection();
        TreeMap<String,String> params = new TreeMap<String,String>();
        params.put("access_token", accessToken);
    ...
    public static WechatResult getTokenByCode(String appid,String secret,String code){
        ...
        String jsonResult = connection.HttpsDefaultExecute("GET",AUTH_TOKEN_PATH,params,"");
